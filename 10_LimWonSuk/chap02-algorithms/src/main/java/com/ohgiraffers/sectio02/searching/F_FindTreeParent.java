...
public class F_FindTreeParent {
    ...
    static int N; // 노드의 개수
    static List<Integer>[] list; // 인티저를 이용하는 배열
    ...
    public static String solution(String input) throws IOException {
        ...
        list = new ArrayList[N + 1]; // List의 참조변수(ArrayList)를 쓰겠다

        // 배열 초기화 후 각노드의 인접 리스트 초기화
        for (int i = 1; i <= N; i++) {
        ...
        // 테스트가 후행 공백을 기대하므로 trim() 제거하여 그대로 반환
        return sb.toString();
    }
    ...
    private static void dfs(int parentNode) {
        ...
        /* 2 단계 : 현재 노드와 연결된 모든 노드 탐색
         * list[parentNode] 에는 parentNode와
         * 연결된 모든 노드 정보가 입력되어 있음
         */
        for (int childNode : list[parentNode]) {

            /* 3 단계 : 아직 방문하지 않은 노드일 때만 추가 탐색 실행
             * == 이미 방문한 노드는 건너뜀
             */
    ...
    /**
     * BFS(너비 우선 탐색)를 이용하여 트리의 부모-자식 관계파악
     * - 루트부터 레벨 순서로 탐색하며 부모-자식 관계를 저장
     * @param root (시작 노드, 1번)
     */
    private static void bfs(int root){
        /* 1 단계 : Queue 초기화 및 루트 노드 추가 */
        Queue<Integer> queue = new LinkedList<>();
        queue.offer(root);
        visit[root] = true;

        /* 2 단계 : Queue가 빌 때까지 반복 */
        while (!queue.isEmpty()){
            /* 현재 처리할 노드를 Queue에서 꺼내기 (FIFO) */
            int parentNode = queue.poll();

            /* 3 단계 : parentNode와 연결된 모든 노드를 탐색 */
            for (int childNode : list[parentNode]){
