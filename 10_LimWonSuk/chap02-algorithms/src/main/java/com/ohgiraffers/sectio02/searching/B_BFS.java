...
public class B_BFS {
    ...
    //  기존 코드는 BufferedReader로 Queue 생성 → 완전 오류
    ...
    public static int solution(String input) throws IOException {
        ...
        // 배추밭에 배추 심기
        for (int i = 0; i < K; i++) {
            st = new StringTokenizer(br.readLine());   //  오타 수정
        ...
        // 배추밭 출력하기
        System.out.println("[[ 배 추 밭 ]]");
        for (int i = 0; i < M; i++) {
            for (int j = 0; j < N; j++) {    //  j < N 이어야 함
        ...
        for (int i = 0; i < M; i++) {
            for (int j = 0; j < N; j++) {
                ...
                if (map[i][j] == 1 && !visit[i][j]) {
                    ...
                    /* BFS 알고리즘을 통해서
                     * 현제 베추와 상, 하, 좌, 우로 연결된 모든 배추를 찾아서
                     * 방문처리 (visit[x][y] = true)
                     * */
                    bfs(i, j); // 묶음 찾기
    ...
    private static void bfs(int x, int y) {
        ...
        while (!q.isEmpty()) {
            ...
            for (int i = 0; i < 4; i++) {
                ...
                // 다음 탐색할 위치를 계산
                cx = node.x + dirX[i];
                cy = node.y + dirY[i];   // **** 오타 수정됨 ****

                // 4단계 : 계산한 위치가  유효한지 검사
                // -> 배추밭 범위를 넘지 않았는지 확인
                // - 방문하지 않은 곳인지 확인
                // - 배추가 있는지 확인
                boolean condition1 = cx >= 0 && cy >= 0 && cx < M && cy < N; // 이조건을 모두 만족해야 범위안에 있다
/*                boolean condition2 = !visit[cx][cy];
                boolean condition3 = map[cx][cy] == 1;*/

                if (condition1 && map[cx][cy] == 1 && !visit[cx][cy]) { // 유효한 경우
