...
public class D_DijkstraAlgorithm {
  ...
  public static String solution(String input) throws IOException {
    ...
    /* 시작 정점 초기화 */
    pq.offer(new Edge(start, 0));   // 시작 정점을 큐에 추가 (거리 0) / 0이 우선순위 제일 높음
    dis[start] = 0;                 // 시작 정점까지의 거리는 0

    /* 다익스트라 메인 루프 */
    while (!pq.isEmpty()) { // 큐에 있던걸 뺌
      System.out.println("우선순위큐 : " + pq); // 큐에 남은게 X -> (1,0) 입력
      ...
      if (nowCost > dis[now]) continue; // 0번의 가중치가 현재것 보다 크면  넘어간다

      /* 3단계: 현재 정점과 연결된 이웃 정점들을 확인 */
      // graph.get(now): 현재 정점(now)에서 출발하는 모든 간선
      for (Edge edge : graph.get(now)) { // 1번 노드를 얻어옴
        /* 거리 갱신 조건 확인 */
        // dis[edge.ver]: 현재까지 알려진 edge.ver까지의 최단 거리
        // nowCost + edge.cost: 현재 정점을 거쳐서 가는 거리
        // 만약 현재 정점을 거쳐가는 것이 더 짧다면?
        if (dis[edge.ver] > nowCost + edge.cost) { // 무한 > 14 -> 무한 자리에 14를 넣음 - > 이런식으로 진행
          /* 더 짧은 경로 발견! 거리 갱신 */
          dis[edge.ver] = nowCost + edge.cost; // 갱신완료
    ...
    for (int i = 2; i < dis.length; i++) {
      if (dis[i] != Integer.MAX_VALUE) {
        ...
        sb.append("impossible"); // 2137383647 21억3천7백83만3천6백4십7 -> impossible로 출력
      }
      sb.append(" ");  // 공백으로 구분
    }
