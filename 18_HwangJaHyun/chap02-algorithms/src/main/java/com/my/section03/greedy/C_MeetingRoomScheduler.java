...
* - 결과는 배정된 회의의 개수를 반환한다.
*
* - 회의실 개수 1개
* - N = 회의 개수
*  각 회의는 [시작시간, 종료시간] 정보를 갖는다.
*
* === 그리디 전략 ===
* - 종료시간이 가장 빠른 회의부터 선택한다.
* -> 회의가 빨리 끝나면 이후에 회의를 선택할 수 있는 남은 시간
* */
public class C_MeetingRoomScheduler {
  public static int solution(String input) throws IOException{
    ...
    // 첫번 째 줄(회의 개수) 읽어와서 저장
    int N = Integer.parseInt(br.readLine());
    
    // 회의 정보(시작, 종료) 기록
    int[][] times = new int[N][2]; // index 0: 시작시간, index 1: 종료시간
    ...
    for(int i = 0; i < N; i++){
      ...
      times[i][0] = Integer.parseInt(st.nextToken()); // 시작
      times[i][1] = Integer.parseInt(st.nextToken()); // 종료
    }
    /* !!!<<종료 시간>> 오름차순 정렬 */
    // Array.sort(Object[]) => Object[int[]], 정렬기준이 정의되어있지 않다.
    ...
    for(int i = 0; i < N; i++){
      /* 현재 회의의 시작 시간이 이전 회의의 종료시간 이후인지 확인
      * 맞을 경우 = 회의를 배정할 수 있는 상태
      * -> 배정된 회의 개수 증가,
