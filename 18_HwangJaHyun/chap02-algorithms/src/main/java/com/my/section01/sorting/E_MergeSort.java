...
public class E_MergeSort {
  ...
  private static void mergeSort(int[] arr, int[] temp, int left, int right) {
    // 분할된 배열의 크기가 2이상인 경우만 수행
    // <-> 분할된 배열의 크기가 1 이하인 경우에는 수행 X
    //  --> if 문 수행 종료 == 분할된 배열이 정렬 완료됨
    if(left < right){
      // 1. 분할 - 중간 지점 계산
  ...
  private static void merge(int[] arr, int[] temp, int left, int mid, int right) {
    ...
    while(leftIndex <= mid && rightIndex <= right){
      ...
      // 임시배열(temp)의 leftIndex가 가리키는 값 <= rightIndex가 가리키는 값
      if(temp[leftIndex] <= temp[rightIndex]){
        arr[current++] = temp[leftIndex++];
      }
      // 임시배열(temp)의 rightIndex가 가리키는 값 <= leftIndex가 가리키는 값
      else{
    ...
    // 오른쪽 배열에 요소가 남아있을 경우에는 별도 처리 필요X
    // -> 이미 정렬된 상태로 볼 수 있음
    while(leftIndex <= mid){
      arr[current++] = temp[leftIndex++];
    }
