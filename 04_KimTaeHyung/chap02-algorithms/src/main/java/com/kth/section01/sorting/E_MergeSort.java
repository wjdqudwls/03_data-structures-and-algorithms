...
/* 병합 정렬
* - 분할 정복(Divide and Conquer) 방식으로 배열을 반씩 나누어(1~2개 요소가 남을 때 까지) 정렬한 후 병합하는 방식
...
public class E_MergeSort {
   ...
   * @param mid 왼쪽 부분 배열의 끝 인덱스(오른쪽 시작 mid + 1)
  ...
  private static void merge(int[] arr, int[] temp, int left, int mid, int right){
    ...
    // 1단계 : 병합할 구간의 배열 요소를 임시 배열(temp)에 복사 -> 원본 수정 전 값을 보존하기 위해서
