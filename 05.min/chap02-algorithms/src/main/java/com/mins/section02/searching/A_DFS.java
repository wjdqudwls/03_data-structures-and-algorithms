...
public class A_DFS {
  ...
  public static Integer solution(String input) throws IOException {
    ...
    for (int i = 0; i < edge; i++) {
      str = new StringTokenizer(br.readLine()); // 공백 기준으로 토큰 나눔
      // 1 3 -> "1", "3"
  ...
  private static void dfsRecursive(int start) {
    ...
    for (int i = 1; i <= node; i++) {
      ...
      if (map[start][i] == 1 && !visit[i]) {
        ...
        /* 3단계 : 찾은 이웃 노드
         *   그 노드와 연결된 이웃 또는 찾는 과정을 반복(재귀 호출)
    ...
    while (!stack.isEmpty()) {
      ...
      for (int i = 1; i <= node; i++) {
        ...
        if (map[current][i] == 1 && !visit[i]) {
          stack.push(i);    // 스택에 추ㅎ가(나중에 방문한 노드)
          visit[i] = true;  // 방문 처리
          count++;          // 감연된 컴퓨터 수 증가
        }
      }
