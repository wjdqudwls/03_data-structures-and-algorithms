...
public class B_BFS {
  ...
  static int M; // 가로 길이
  static int N; // 세로 길이
  static int K; // 배추(노드) 개수

  // 배추 밭을 나타낼 인접 행렬(2차원 배열)
  static int[][] map;
  ...
  static int cx, cy;  // current X, current Y

  // BFS 방식 으로 해결하기 위한 Queue 생성
  static Queue<Node> queue = new LinkedList<>();


  /**
   * 배추밭에 필요한 지렁이의 수를 구하는 메서드
   * @param input 입력 데이터(배추밭 크기, 배추 갯수, 배추 위치)
   * @return 필요한 지렁이의 수
   * @throws IOException
   */
  public static int solution(String input) throws IOException {
    ...
    // count 0으로 초기화
    // static의 공유 특징 때문에 테스트 마다 값이 누적 됨
    count = 0;

    /* 배추밭 전체를 탐색하여, 연결된 배추 묶은 찾기 */
    for (int i = 0; i < M; i++) {
      for (int j = 0; j < N; j++) {
...
//      boolean condition2 = !visit[cx][cy];
//      boolean condition3 = map[cx][cy] == 1;

      if(condition1 && !visit[cx][cy] && map[cx][cy] == 1){ // 유효한 경우
        queue.offer(new Node(cx,cy)); // 큐에 추가 -> 다음 탐색 후보로 등록
