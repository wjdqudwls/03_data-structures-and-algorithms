...
public class F_FindTreeParent {
  ...
  /* 트리의 부모 노드 찾기
  * - 무 방향 그래프로 주어진 트리의 간선 정보를 이용하여
  *   각 노드의 부모 노드 찾기
  * - DFS, BFS를 이용하여 탐색
  * - 인접 리스트(각 노드에 연결된 노드(== 간선 정보) 저장 )
  * */
  static int N; // 노드의 개수
  static List<Integer>[] list; // 인접 리스트 ( Integer만 저장하는 리스트의 배열)
  ...
  public static String solution(String input) throws IOException {
    ...
    // 배열 초기화, 인덱스 번호 == 노드 번호
    parent = new int[N+1];// 0~N-1 까지가 아니라, 1~N 까지 쓸 예정
    ...
    for(int i = 1; i < N+1; i++){
      ...
      // list[1] : 2,3 -> 1번 노드가 2,3번 노드와 연결되어 있음.
      // list[2] : 1,4 -> 2번 노드가 1,4번 노드와 연결되어 있음.
    }

    /* 간선 정보 입력 (N-1개의 간선)
    : Tree는 N개의 노드, N-1의 Edge를 가짐 */
...
//    dfs(1);
  ...
  private static void dfs(int parentNode){
    ...
    // list[parentNode]에는 parentNode와 연결된 모든 노드 정보가 입력 되어 있음
   ...
   * - root 부터 레벨 순서로 탐색하며 부모-자식 관계를 저장
   * @param root 시작 노드, 1번
   */
