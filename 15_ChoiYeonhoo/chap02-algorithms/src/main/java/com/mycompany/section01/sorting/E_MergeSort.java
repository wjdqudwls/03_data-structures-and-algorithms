...
public class E_MergeSort {
  ...
  private static void mergeSort(int[] arr, int[] temp, int left, int right) {
    ...
    // <-> 분할된 배열의 크기가 1 이하인 경우에는 수행을 하지 않음
    // --> if 수행 종료 == 분할된 배열이 정렬 완료됨
  ...
  private static void merge(int[] arr, int[] temp, int left, int mid, int right) {
    ...
    // 1단계 : 병합할 구간의 배열의 요소를 임시 배열(temp)에 복사
    // -> 원본 수정 전 값을 보존하기 위해서
    ...
    int leftIndex = left; // 왼쪽 배열 시작 위치
    int rightIndex = mid + 1; // 오른쪽 배열 시작 위치
    int currentIndex = left; // 원본 배열 시작 위치

    // 3단계 : 두 부분 배열을 비교하면서 작은 값부터 원본배열에 저장
    // -> 이 단계에서 정렬이 됨
    ...
    while (leftIndex <= mid && rightIndex <= right) {
      ...
      // (안정 정렬을 위해 같은 값이면 왼쪽이 우선)
      if (temp[leftIndex] <= temp[rightIndex]) {
        arr[currentIndex++] = temp[leftIndex++];
      }
      // 임시 배열의 rightIndex가가 가리키는 값이
      // leftIndex가 가리키는 값 보다 같거나 작은 경우
      else {
